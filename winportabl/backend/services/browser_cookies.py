...
def sync_skyshowtime_from_browser_cookies(
    ...
    try:
        ...
        return False
def sync_ytdlp_cookies(extracted: Dict[str, List[Dict[str, str]]]) -> bool:
    """Extract youtube.com and google.com cookies and write to ytdlp_cookies.txt."""
    yt_cookies = extracted.get("youtube.com", []) + extracted.get("google.com", [])
    if not yt_cookies:
        return False
        
    try:
        from backend.services.ytdlp_common import get_ytdlp_cookies_path
        cookie_file = get_ytdlp_cookies_path()
        ...
        cookie_file.write_text("\n".join(lines), encoding="utf-8")
        logger.info("YouTube cookies auto-synced to ytdlp_cookies.txt.")
        return True
    except Exception as e:
        logger.error("Failed to write ytdlp_cookies.txt: %s", e)
        return False
...
def sync_all_supported_services() -> Dict[str, Any]:
    ...
    targets = list(DOMAIN_TO_SERVICE.keys()) + ["youtube.com", "google.com"]
    ...
    # Auto-sync YouTube/Google cookies to ytdlp_cookies.txt
    sync_ytdlp_cookies(extracted)
