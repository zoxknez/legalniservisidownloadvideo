...
def execute_job(payload: Dict[str, Any], log_fn: LogFn) -> bool:
    ...
    if service == "voyo":
        ...
        return run_voyo_job(action, params, log_fn)
    if service in ("hbomax", "hbo"):
        from backend.jobs.hbo_job import run_hbo_job
        return run_hbo_job(action, params, log_fn)
    if service == "sniffer":
        from backend.jobs.sniffer_job import run_sniffer_job
        return run_sniffer_job(action, params, log_fn)
