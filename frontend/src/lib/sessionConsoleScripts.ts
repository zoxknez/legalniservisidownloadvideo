...
export const SESSION_IMPORT_PLACEHOLDERS: Record<string, string> = {
  ...
  hrti: "JSON sa token i customer_id (ili token ako JWT sadrži CustomerId)...",
...
export function buildSessionConsoleScripts(): Record<string, { title: string; code: string }> {
  ...
  return {
    ...
    hrti: {
      title: "HRTi",
      code: `fetch('${bridgeUrl}',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({batch:{hrti:${hrtiCollector}},source:'console'})}).then(r=>r.json()).then(j=>console.log(j.success?'✓ Poslato u app':j));`,
    },
    rtsplaneta: {
      title: "RTS Planeta",
